...
func TestBindDataAPI(t *testing.T) {
	...
	t.Run("GET Query BadReqeust", func(t *testing.T) {
		resp, err := http.Get(fmt.Sprintf("%s/user?id=1&password=qwer", ts.URL))
		if err != nil {
	...
	t.Run("POST JSON OK", func(t *testing.T) {
		...
		if resp.StatusCode != http.StatusOK {
			t.Logf("Expected status code %v, got %v", http.StatusOK, resp.StatusCode)
		}
		printResponseBody(resp)
	})
	...
	t.Run("PUT URI JSON OK", func(t *testing.T) {
		...
		buff := bytes.NewBuffer(b)
		client := &http.Client{}
		req, _ := http.NewRequest(http.MethodPut, fmt.Sprintf("%s/user/1/wrongName", ts.URL), buff)
		resp, err := client.Do(req)
		if err != nil {
			t.Fatalf("Expected no error, got %v", err)
		}
